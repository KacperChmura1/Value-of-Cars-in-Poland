...
def model_selection(brand):
    for i in range(0,len(brands_all)):
       if brand == brands_all[i]:
            ...
            return model_c
def predict_car_value_ml(df2):
       model = pickle.load(open("ML_models/ridge", 'rb'))  
       ml_model = st.sidebar.selectbox("Model choose", {"Ridge","Losso","Stochastic Gradient Descent","Gradient Boost","Adaptive Boost","Random Forest"})
       if ml_model == "Ridge":
            model = pickle.load(open("ML_models/ridge", 'rb'))  
       elif ml_model == "Stochastic Gradient Descent":
            model = pickle.load(open("ML_models/sgd", 'rb'))
       elif ml_model == "Lasso":
            model = pickle.load(open("ML_models/lasso", 'rb'))
       elif ml_model == "Gradient Boost":
            model = pickle.load(open("ML_models/Gradient_Boost", 'rb'))
       elif ml_model == "Adaptive Boost":
            model = pickle.load(open("ML_models/Ada_Boost", 'rb'))
       elif ml_model == "Random Forest":
            model = pickle.load(open("ML_models/random_forest", 'rb'))
       ...
       if ok:  
              ...
              true_value = f'<p style="font-family:Courier; color:Green; font-size: 30px;">Predicted Value: {round(car_price)}PLN</p>'
              st.markdown(ml_model)
